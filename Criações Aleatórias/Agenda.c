...
if(file){
	...
	for(i = 0; i < quant; i++){
		fputs(c[i]->nome, file);
		fputs("\n", file);
		fprintf(file, "%d %d %d\n%s\n", c[i]->dia, c[i]->mes, c[i]->ano, c[i]->detalhes);
...
int ler_arquivo(Contatus **c, char arq[]){
	...
	if(file){
		fscanf(file, "%d\n", &quant);
		for(i = 0; i < quant; i++){
			//fgets(novo->nome, 60, file);
			fscanf(file, "%60[^\n]", novo->nome);
			fscanf(file, "%d %d %d\n%s\n", &novo->dia, &novo->mes, &novo->ano, &novo->detalhes);
			c[i] = novo;
			novo = malloc(sizeof(Contatus));
		}
		fclose(file);
	} else {
